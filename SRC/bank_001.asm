...
; Draw energy tanks {
    ...
    jr z, .else_A
        ...
        ld b, a
        ld hl, hHUD_tank1
        ld a, tile_emptyTank ; Empty tank
        ...
        jr z, .endIf_A
            ...
            ld b, a
            ld hl, hHUD_tank1
            ld a, tile_fullTank ; Filled tank
...
.waveBranch: ;{ Wave
    ...
    .else_I:
			...
			;veil ricochetFieldBlock
				cp $20
				jr nc, .else_J
				cp $1c
				jr c, .else_J
					; if not on surface, this tile is inert due to ship gfx
					ld a, [saveBuf_bgGfxSrcHigh]
					cp $60	;high byte for surface tileset, all are in bank 7, guessing all take at least 100 bytes...
					jr z, .else_J	;is actually just the end of my fancy custom checks, not the fuel ones specifically :/
						call destroyBlock
						jp .nextProjectile
	.else_J:
...
.missileSpeedTable: ; 01:51A1 - Missile speed table (first value in array is unused)
    db $00, $01, $01, $01, $02, $02, $02, $02, $03, $03, $03, $03, $03, $04, $04, $04
    db $04, $04, $04, $05, $05, $05, $05, $05, $05, $05, $06, $06, $06, $06, $06, $06
    db $06, $FF
...
.commonBranch: ; Common projectile code
    ...
    jp z, .else_Q
					ldh a, [hBeam_type]  
					cp $04 ; Plasma destroys fuel blocks
					jr nz, .checkSkipAir
					call getTileIndex.projectile
					jr .checkAirDestructibleToo
        ...
        jr nc, .else_Q
			...
			;veil ricochetFieldBlock
				...
				jr c, .else_notField
					...
					ld a, [saveBuf_bgGfxSrcHigh]
					cp $60	;high byte for surface tileset, all are in bank 7, guessing all take at least 100 bytes...
					jr z, .notFuel	;is actually just the end of my fancy custom checks, not the fuel ones specifically :/
						push hl
						ld hl, $dd01
						ld a, [projectileIndex]
						add a, l
    ...
    .projectileLoop:
        ...
        jp z, .nextProjectile
            ...
            .else_A:
					...
					jr nz, .notWave
						...
						jr .normal
							...
							.normal:
								...
								ldh [hSpriteAttr], a
				.notWave:
					;if plas or spazer, load specific tile of two available
					ld a, [samusBeam]
					cp $04
					jr z, .isPlas
						cp $03
                ...
                jr nz, .endIf_A
					;if plas load specific tile, otherwise load the vert if shooting vert
					.isPlas:
...
; Row index is based off of your facing direction of the samus_shotDirectionPriorityTable
    ...
    db $00, $00
    db $18, $1B ; Right - The opposing L/R directions are used when
    db $04, $07 ; Left  - firing backwards while damage boosting
    db $10, $10
    db $0E, $12 ; Up
    db $10, $10
    db $10, $10
