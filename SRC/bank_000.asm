...
;  Looks like 
    ...
    jr z, .else_B
        ...
        jr .endIf_B
    .else_B:
		; Handle pause map updates if the level data isn't being updated
		ld a, [loadNewMapFlag]
		cp $00
		jr z, .noPauseMapUpdate
			ld a, BANK(VBlank_loadPauseMapDuringTransition)
			ld [rMBC_BANK_REG], a
			call VBlank_loadPauseMapDuringTransition
			jr .endIf_B
		.noPauseMapUpdate:
		; m2maps: skip updating gameplay status bar if game is paused
			...
			jr z, .endIf_B
				;veil animated tile updates experiment
				ld a, [itemCollected]
				cp $00
				jr nz, .endIf_B
					;RT-55J correction in lieu of callFar which shouldn't be done during vblank
					ld a, BANK(VBlank_animateTiles)
					ld [rMBC_BANK_REG], a
					call VBlank_animateTiles
					cp $00
					jr z, .doVBlank_UpdateStatusBar
						call loadAnimatedTilesToVram
						jr .endIf_B
		; end m2maps block
		.doVBlank_UpdateStatusBar:
...
; Load various variables from saveBuf to their working copies
    ...
    ld [metroidCountDisplayed], a
        ...
        ; m2maps: account for SRAM for item count
            ...
            ld [mapItemsFound], a
            ld a, [saveBuf_newItemsEquipped]
            ld [newItemsEquipped], a
        ; end m2maps block
    ; Clear variables
    xor a
...
; Handle right input
    ...
    jr z, .endIf_E
        ...
        jr z, .else_F
            ...
            ld a, $01
            ld [samusFacingDirection], a
			;veil modify turning if speedbooster enabled right vanilla is $02
            ld a, $02
...
; Handle left input
    ...
    jr z, .endIf_G
        ...
        jr z, .else_H
            ...
            ld a, $00
            ld [samusFacingDirection], a
			;veil modify turning if speedbooster enabled left vanilla is $02
            ld a, $02
            ld [samus_turnAnimTimer], a
				ld a, [samusItems]
				bit itemBit_UNUSED, a
...
;veil combined walk speed fetcher
veil_Samus_Speed:
; Set speed
    ; Water speed
    ld b, $01
...
.morph_Entry:
        ...
        ld a, [samusItems]
        bit itemBit_varia, a
		;veil swapped z to nz
        jr nz, .else
            ; Varia (veil normal) walking speed
				inc b
				; speedBoost
				bit itemBit_UNUSED, a
				jr nz, .withSpeed
            jr .endIf
        .else:
            ; Normal (veil varia) walking speed (1.5 px/f)
				; speedBoost
				bit itemBit_UNUSED, a
				jr z, .notSpeed
					inc b
...
.start:
    ; Set speed
	call veil_Samus_Speed.morph_Entry
.start_Spider:
    ; Set facing direction
    ld a, $01
    ld [samusFacingDirection], a
...
; Top left side
    ...
    jr z, .endIf_A
			;veil dont set water contact flag if gravity
...
.projectile: ; 00:2266 - Entry point for beam routines
    ...
    ; Combine results of the two read attempts
    and b
		;veil enemy destroyBlock:
		push af
		ld a, [enemyBreakBlockFlag]
		cp a, $00
		jr z, .dontBreak
			pop af
			push af
...
.endDoorScript:
    ...
    ld [wramUnknown_D0A8], a
            ...
            ; m2maps: set new map flag
;				ld [loadNewMapFlag], a
;				callFar updateLocatorPosition_farCall
            ; end m2maps block
...
applyDamage: ;{ This procedure has multiple entry points
    ...
    .acid: ; 00:2F4A
        ...
        ld b, a
			;veil ignore acid damage if varia
			ld a, [samusItems]
			bit itemBit_varia, a
			jr z, .no_Varia
				ld a, $00 ; This is supposed to be a jump table index (behavior is unused)
				ld [acidContactFlag], a
				jr .endIf_B
		.no_Varia:
        ; Damage once every 16 frames
...
;               pop hl
		...
		push bc ; Save item value
			...
			; Transfer graphics
			call beginGraphicsTransfer
		pop bc ; Restore Item value

    ; Jump to pick-up specific routine
    ld a, b
    dec a
    rst $28
	;veil swaps varia and springball index
...
pickup_plasmaBeam: ;{
    ...
    ld [samusBeam], a
	ld a, [newItemsEquipped]
    set itemBit_Plasma, a
    ld [newItemsEquipped], a
	ld [saveBuf_newItemsEquipped], a
    ; Load graphics
...
;veil newItemsEquipped collection
pickup_speedBoost: ;{
    ; Set item bit
    ld a, [samusItems]
    set itemBit_UNUSED, a
    ld [samusItems], a
jp handleItemPickup_end ;}
...
pickup_missileTank: ;{
;veil refill all missiles to max
...
ret ;}
m2maps:
loadAnimatedTilesToVram:
    ld a, [hl+]
	jr loadGame_copyItemToVram.dontWriteBankRegMirror
...
destroyRespawningBlock_farCall:
	callFar destroyRespawningBlock
	switchBankVar $02 ; Enemy AI bank
ret